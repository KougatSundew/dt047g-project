...
Client::Client(QHostAddress ip) : m_ip(ip), m_loggedIn(false) {
    m_clientSocket = new QTcpSocket(this); //Change to unique_ptr for RAII
...
void Client::sendMessage(const QString &text) {
    if(text.isEmpty()) return; //Exception for empty text
    m_clientSocket->write("<LOGIN><Joel><1337>");
    /*
...
//Signals
void Client::messageReceived(const QString &sender, const QString &text) {
